  ...
  head: () => ({
    meta: [
      { title: "Ravitej — Original Charcoal, Paintings & Sketches" },
      {
        name: "description",
        content:
          "Original black & white artworks by Ravitej. Charcoal, oil paintings, and graphite sketches available worldwide.",
      },
    ],
  }),
  component: Home,
...
const MEDIUM_META: Record<string, { index: string; accent: string; description: string }> = {
  charcoal: {
    index: "01",
    accent: "from-zinc-900 via-zinc-800/30 to-transparent",
    description: "Raw marks on archival paper. Shadow made solid.",
  },
  paintings: {
    index: "02",
    accent: "from-stone-900 via-stone-700/20 to-transparent",
    description: "Oil on canvas. Emotion rendered in paint.",
  },
  sketches: {
    index: "03",
    accent: "from-neutral-900 via-neutral-600/20 to-transparent",
    description: "Graphite studies. Gesture captured in an instant.",
  },
};
    ...
    <div className="min-h-screen">
      ...
      <section className="relative h-[100svh] overflow-hidden">
        ...
        <div className="relative z-10 h-full flex flex-col items-center justify-center px-4 sm:px-6 text-center">
          ...
          <h1
            className="text-display text-[clamp(2.8rem,10vw,11rem)] leading-[0.9] animate-fade-up"
          ...
          <p
            className="mt-8 sm:mt-10 max-w-md text-xs sm:text-sm text-muted-foreground leading-relaxed animate-fade-up px-4"
            style={{ animationDelay: "0.3s" }}
          >
            Every artwork is drawn by hand and crafted to tell a story.
          </p>
          <div
            className="mt-10 sm:mt-12 flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-6 animate-fade-up w-full px-4 sm:px-0"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-5">
          {MEDIUMS.filter((m) => m.id !== "all").map((m) => {
            const meta = MEDIUM_META[m.id] ?? {
              index: "—",
              accent: "from-zinc-900 to-transparent",
              description: "",
            };
            ...
            return (
              ...
              >
                <div
                  className={`absolute inset-0 bg-gradient-to-br ${meta.accent} opacity-60 group-hover:opacity-100 transition-opacity duration-700`}
                />
                ...
                <div className="relative z-10">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="w-0 group-hover:w-8 overflow-hidden transition-all duration-500 text-foreground/60 text-xs">
                      →
                    </span>
                  </div>
                </div>
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-12 py-16 md:py-20">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-10 md:mb-12">
          <div>
            <p className="text-[10px] sm:text-xs uppercase tracking-[0.3em] text-muted-foreground mb-2 sm:mb-3">
              — Selected works
            </p>
          ...
          <Link
            to="/gallery"
            className="text-[10px] sm:text-xs uppercase tracking-[0.25em] text-muted-foreground hover:text-foreground transition-colors"
          >
            See all →
          </Link>
        </div>
        ...
        <div className="relative overflow-hidden border border-border/60 p-8 sm:p-12 md:p-20 text-center bg-card/30">
          ...
          <div className="relative z-10">
            ...
            <p className="text-sm sm:text-base text-muted-foreground max-w-xl mx-auto mb-8 sm:mb-10 leading-relaxed px-2">
              Commissions are open for portraits, abstract works, and large-scale paintings. Tell me
              about your space and we'll start the conversation.
            </p>
            <Link
              to="/contact"
